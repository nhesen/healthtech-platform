...
export async function api<T>(path: string, role: DemoRole): Promise<T> {
  const res = await fetchApi(path, { headers: { "X-Demo-User": role }, cache: "no-store" });
  return checked<T>(res);
}
export async function mutate<T>(path:string,role:DemoRole,body?:unknown):Promise<T>{
 const res=await fetchApi(path,{method:"POST",headers:{"X-Demo-User":role,"Content-Type":"application/json"},body:body?JSON.stringify(body):undefined});
 return checked<T>(res);
}
export async function request<T>(path:string,role:DemoRole,method:string,body?:unknown):Promise<T>{
 const res=await fetchApi(path,{method,headers:{"X-Demo-User":role,"Content-Type":"application/json"},body:body?JSON.stringify(body):undefined});
 return checked<T>(res);
}
