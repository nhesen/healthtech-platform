...
export default function Home() {
  ...
  const firstName = data.overview.patient.name.split(" ")[0];
  const attention = data.trends.conflicts.length > 0 || data.trends.trends.some(x => x.trend === "increasing");
  return <Screen refreshing={state.loading} onRefresh={state.reload}>
    <PageHeader title={`Salam, ${firstName}`} subtitle="Here is your health overview" action={{ icon: "bell-outline", badge: unread, onPress: () => router.push("/notifications") }}/>
    <View style={styles.statusGrid}><Card style={styles.statusCard}><Text style={styles.label}>Health Status</Text><View style={styles.statusLine}><View style={[styles.dot, attention && { backgroundColor: colors.warning }]}/><Text style={[styles.stable, attention && { color: colors.warning }]}>{attention ? "Needs review" : "Stable"}</Text></View></Card><Card style={styles.statusCard}><Text style={styles.label}>New Insights</Text><Text style={styles.big}>{data.overview.insight_count}</Text></Card></View>
    <Card title="Insurance"><Text style={styles.cardLead}>{data.overview.patient.insurance_plan}</Text><Text style={styles.muted}>{data.trends.care_navigation?.suggested_specialty ? `Active · ${data.trends.care_navigation.suggested_specialty} suggested` : "Active · synthetic demo coverage"}</Text></Card>
